...
# Quick mode: set env var QUICK_TRAIN=1 to run a single, faster experiment
QUICK_TRAIN = os.getenv('QUICK_TRAIN', '0') == '1'
...
(Linear)
    ...
    """
    # Connect to MLflow tracking server. If server is unavailable,
    # fall back to a local file-backed mlruns directory so training still runs.
    try:
        mlflow.set_tracking_uri(MLFLOW_URI)
        mlflow.set_experiment(EXPERIMENT_NAME)
    except Exception as e:
        print(f"Warning: couldn't reach MLflow server at {MLFLOW_URI}: {e}")
        local_mlruns = Path.cwd() / "mlruns"
        mlflow.set_tracking_uri(local_mlruns.resolve().as_uri())
        mlflow.set_experiment(EXPERIMENT_NAME)
        print(f"Switched to local MLflow tracking at {local_mlruns}")
    ...
    with mlflow.start_run(run_name=run_name) as run:
        ...
        cv_folds = 5 if not QUICK_TRAIN else 2
        cv_n_jobs = -1 if not QUICK_TRAIN else 1
        ...
        plots_dir = Path.cwd() / 'mlflow_plots'
        plots_dir.mkdir(exist_ok=True)

        fi_path = plot_feature_importance(
            pipeline, FEATURE_COLS, str(plots_dir / 'feature_importance.png')
        )
        if fi_path:
        ...
        avp_path = plot_actual_vs_predicted(
            y_val, y_pred,
            str(plots_dir / 'actual_vs_predicted.png'),
            mae,
        )
...
if __name__ == '__main__':
    ...
    if QUICK_TRAIN:
        print('QUICK_TRAIN=1 detected — running a single fast experiment')
        train_and_log(
            run_name='gbr-quick',
            model_type='gbr',
            hyperparams={'n_estimators': 50, 'learning_rate': 0.1, 'max_depth': 3},
            data_path=DATA_PATH,
            save_model=False,
        )
    else:
